...
try {
  const url = 'https://api.eyepop.ai/pops/'+pop_uuid+'/config?auto_start=true';
...
try {
  //ai.crmLog(popConfig)
  const url =popConfig.base_url +  '/pipelines/' + popConfig.pipeline_id + '/source?mode=preempt&processing=sync';
  const body = (JSON.stringify({
      "sourceType": "URL",
      "url": image_url
    }))
